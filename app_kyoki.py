...
# ワードクラウド、出現頻度表、共起ネットワークの処理をサイドバーに表示
st.sidebar.markdown("# 可視化のオプション")

# MeCabのインスタンスを作成
tagger = MeCab.Tagger("")  # MeCabのインスタンスを作成

if uploaded_file is not None:
    raw_data = uploaded_file.getvalue()
    encoding = chardet.detect(raw_data)['encoding']  # ファイルのエンコーディングを検出
    text = raw_data.decode(encoding)  # テキストをデコード
    ...
    if option == "ワードクラウド":
        ...
        # フォントのパスを指定
        # font_path = '/content/drive/MyDrive/ONGAESHI/ipaexg00401/ipaexg.ttf'
        # フォントプロパティを設定
